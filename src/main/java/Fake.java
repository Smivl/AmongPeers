...
public class Fake extends Application {
    ...
    public static RemoteSpace positionSpace;

    public static void main(String[] args) throws InterruptedException {


        try {
            positionSpace = new RemoteSpace("tcp://localhost:9001/position?keep");
        }catch ( Exception e){
    ...
    public void start(Stage primaryStage) {
        ...
        this.player2.setCenterX(250);
        this.player2.setCenterY(250);
        this.player2.setFill(Color.BLACK);
        ...
        Timeline t = new Timeline(new KeyFrame(Duration.millis(16), e-> {
            ...
            if (!Arrays.equals(this.position, new double[]{this.circle.getCenterX(), this.circle.getCenterY()})){
                try {
                    positionSpace.put("Client", this.circle.getCenterX(), this.circle.getCenterY());
                } catch (Exception ec) {
    ...
    private void handleOtherPlayer(){
        while(true){
            try{
                Object[] res = positionSpace.get(new ActualField("Server"), new FormalField(Double.class), new FormalField(Double.class));
